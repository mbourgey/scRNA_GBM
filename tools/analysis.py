# Note by Couturier: this is the workhorse of the velocyto package (La Manno et al., http://velocyto.org). Small modifications were made to allow the removal of cells from the dataset after normalization. 
...
class VelocytoLoom:
    ...
    def filter_cells(self, bool_array: np.ndarray) -> None:
        ...
        try:
            ...
            pass
	# This added (couturier)	
        try:
            self.S_sz, self.U_sz = (X[:, bool_array] for X in (self.S_sz, self.U_sz))
        except:
            pass
        try:
            self.Sx_sz, self.Ux_sz = (X[:, bool_array] for X in (self.Sx_sz, self.Ux_sz))
        except:
            pass
        try:
            self.Sx, self.Ux = (X[:, bool_array] for X in (self.Sx, self.Ux))
        except:
            pass
        try:
            self.S_norm, self.U_norm = (X[:, bool_array] for X in (self.S_norm, self.U_norm))
        except:
            pass
        try:
            self.Upred, self.velocity, self.delta_S, self.Sx_sz_t = (X[:, bool_array] for X in (self.Upred, self.velocity, self.delta_S, self.Sx_sz_t))
        except:
            pass
	# 
    ...
    def filter_genes(self, by_detection_levels: bool=False, by_cluster_expression: bool=False,
        ...
        # added by couturier
        try:
            self.U_norm = self.U_norm[tmp_filter, :]
            self.S_norm = self.S_norm[tmp_filter, :]
        except:
            pass
        try:
            self.Ux = self.Ux[tmp_filter, :]
            self.Sx = self.Sx[tmp_filter, :]
        except:
            pass
        try:
            self.Ux_sz = self.Ux_sz[tmp_filter, :]
            self.Sx_sz = self.Sx_sz[tmp_filter, :]
        except:
            pass        
        #
